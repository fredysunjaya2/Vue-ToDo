...
import { taskToast } from "@/composables/sweetalert.js";
import { useTodosStore } from "@/stores/todos";

let formValid = ref(false);
const isDialogShown = ref(false);

const { addTask, updateTask } = useTodosStore();
...
const updateExistingTask = async () => {
  try {
    ...
    updateTask(props.taskItem, payload);
            ...
            <v-row>
              <v-col>
                <v-form
                  ...
                  @submit.prevent="
                    taskOldTitle === '' ? addNewTask() : updateExistingTask()
                  "
                  v-model="formValid"
                >
                  <v-text-field
                  ...
                  <v-text-field
                    ...
                    placeholder="Type Task"
                    type="text"
                    v-model="taskNewTitle"
                    :rules="rules.taskRules"
                    autofocus
                  >
                  </v-text-field>
                  <v-btn v-if="taskOldTitle === ''" type="submit"> Add </v-btn>
                  <v-btn v-else type="submit"> Confirm </v-btn>
                  <v-btn
                    @click="
                      isDialogShown = false;
                      taskNewTitle = taskOldTitle;
                    "
                  >
                    Cancel
                  </v-btn>
